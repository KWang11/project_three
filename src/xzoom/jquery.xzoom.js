;(function($){
	$.fn.xzoom = function(options){
		// 插件默认参数
		var defaults = {
			width:300,    //用户外界传入唯一的宽度，其他宽度数据，在此基础上，除以比例因子，进行调整，都应用这个宽度
			height:300,  //用户外界传入唯一的高度，其他高度数据，在此基础上，除以比例因子，进行调整，都应用这个高度
			gap:200,      //右图距离左边图的距离
			position:'right',  //默认放大镜框在右侧 , left在左侧， bottom 在下侧
			
		};

		// 扩展默认值
		var opt = $.extend({},defaults,options);    //  opt是一个扩展对象，里面存放很多数据
